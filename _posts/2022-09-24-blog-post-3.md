...
=====
We are almost there. We now can work out the diffusion in its descrete form, but we need to bear in mind that the redox reactions also modifies the concentration of $\mathrm{Ox}$ and $\mathrm{Red}$ on the surface of electrode, and the redox current is the one my collegues record in the CV measurement.
