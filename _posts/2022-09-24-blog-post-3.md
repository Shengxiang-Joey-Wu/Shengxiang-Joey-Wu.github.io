...
If, for simplicity, we treat the diffusion in one dimension only, that is, we assume both Ox and Red only diffuse perpendicular to the electrode surface and the electrode surface is perfectly smooth, then we can use the Fick's second law of diffusion to describe to diffusion of Ox and Red from and approach to surface.
