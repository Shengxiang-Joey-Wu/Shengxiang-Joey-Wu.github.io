...
If, for simplicity, we treat the diffusion in one dimension only, that is, we assume both $\mathrm{Ox}$ and $\mathrm{Red}$ only diffuse perpendicular to the electrode surface and the electrode surface is perfectly smooth, then we can use the Fick's second law of diffusion (eq. 8) to describe diffusion of $\mathrm{Ox}$ and $\mathrm{Red}$ from and approach to surface.
